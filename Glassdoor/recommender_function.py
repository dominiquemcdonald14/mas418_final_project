...
def recommend(df, location, skills, education):
    """
    Given a pandas DataFrame with columns for salary, location, Python skill, R skill, and company name, as well
    as a location input and a list of skills, returns the top 5 salary observations for the specified location
    and skills.
    """
    # filter DataFrame to only include observations from specified location and with specified skills
    loc_recommend_df = df[(df['Location'] == location) & (df['Python'].isin(skills) | df['R'].isin(skills) | df['SQL'].isin(skills)) & (df['Bachelor_Degree'].isin(education) | df['Master_Degree'].isin(education) | df['Ph_D'].isin(education))]
    
    # sort DataFrame by salary in descending order and return top 5 observations
    top_salaries = loc_recommend_df.sort_values(by='average_salary', ascending=False)
    
    print(loc_recommend_df.shape[0])
    
    return top_salaries[['companyName', 'company_starRating', 'company_offeredRole' , 'average_salary', 'requested_url']]

#location is string of state abbreviation or remote
#skills can be single string or list: ['','']
#education can be single string or list: ['','']

#recommend(df,'MO', ['R', 'SQL', 'Python'], ["Bachelor's Degree"])